# Прерванный полет 8
#  добавление обьекта Game  до полной программы
...
class Game(object):
    ...
    def advance(self):
        ...
        # зарезервированное пространство вокруг кораб
        BUFFER = 150
     
        # создание новых астероидов
        for i in range(self.level):
            # вычислим х и у, чтобы от корабля они отстояли минимум на BUFFER пикселей

            # сначала выберем минимальные отступы по горизонтали и вертикали
            x_min = random.randrange(BUFFER)
            y_min = BUFFER - x_min

            # сходя из этих минимумов, сгенерируем расстояния от корабля по горизонтали и вертикали
    ...
    def end(self):
        """ Завершает игру. """
        # 5-секундное отображение 'Игра закончена'
        end_message = games.Message(value = "Игра закончена",
                                    size = 90,
                                    color = color.red,
                                    x = games.screen.width/2,
                                    y = games.screen.height/2,
